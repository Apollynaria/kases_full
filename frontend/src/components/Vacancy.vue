        ...
        <div class="container-fluid text-center g-0">
            <div class="m-auto">
                <div class="zag d-flex row">
                    <div class="container-fluid h1-00 align-self-center zag-txt">ВАКАНСИИ</div>
                </div>
                <div class="me-3 mt-2 mb-4" v-if="vacancies.length > 0">

                    <div class="row ms-3 justify-content-center">
                        <div class="col-sm-12 col-md-6 col-lg-6 col-xl-4 col-xxl-4 col-12 p-3"
                            v-for="(vacancy, index) in vacancies" :key="index">
                            <div class="card h-100">
                                <div class="card-body h-100 d-flex flex-column">
                                    <h3 class="card-title color-red big-font">
                                        {{ vacancy.name }}
                                        <hr class="border border-1 mt-2 border-danger opacity-100 mb-2 m-auto w-100">
                                    </h3>
                                    <p class="card-text color-black">{{ vacancy.description }}</p>
                                    <div class="btn btn-danger f-14 mt-auto card-btn"><i
                                            class="bi bi-telephone-fill"></i>&nbsp;{{ vacancy.phone }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="p-2" v-else>
                    <p class="f-16 text-center color-red">
                        Актуальные вакансии отсутствуют.
                    </p>
                </div>
...
export default {
    ...
    methods: {
        getVacancies() {
            http
                .get("/listShowingVacancies/page=" + this.page)
                .then(response => {
                    this.countPage = response.data.totalPages;
                    this.vacancies = response.data.content;
